  ...
  return self.buildQueryPlan(query, function (err, plan) {
    ...
    Object.keys(plan.colQueries).forEach(function (key) {
      ...
      if (!queryPlan.$limit) {
        ...
        Object.keys(queryPlan.$project).forEach(function (key) {
          var column = queryPlan.$project[key];
          var shortCol = column.substring(0, column.indexOf(')  ')) + ')';
          var endingCol = key;
          sql += 'top(' + (column === 'time' ? '_t' : shortCol) + ',' + queryPlan.$limit + ') as ' + endingCol + ',';
        });
      }
      ...
      var call = function (callback) {
        self.client.query(sql, function (err, result) {
          if (err) {
            if (err.indexOf('Couldn\'t find series: ') > -1)
              result = []; //allow this error to fly
            else
              return callback(err);
          }
