...
InfluxDBProvider.prototype.buildQueryPlan = function (query, callback) {
  ...
  query.metrics.forEach(function (metric) {
    ...
    if (!metric.formula && metric.collection) {
      ...
      _$project[metric.key] = metric.aggregation + '(' + (metric.attribute || metric.key) + ') as ' + metric.key;
    ...
    async.parallel(calls, function (err) {
      ...
      Object.keys(results).forEach(function (key) {
        ...
        if (r.length == 0) {
          ...
          var points = r[0].points;

          points.forEach(function (point) {
            var doc = {};
            columns.forEach(function (col, i) {
              if (col === 'time')
                doc.timestamp = new Date(point[i]);
              else
                doc[col] = point[i];
            });
      ...
      //console.log(require('util').inspect(result.documents, {depth: null, colors: true}));
      return callback(null, result);
    });
  });
};
